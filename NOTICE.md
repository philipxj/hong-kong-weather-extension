# Notice

Hong Kong Weather is an independent, unofficial browser extension. It is not affiliated with or endorsed by the Hong Kong Observatory, the Government of the Hong Kong Special Administrative Region, or DATA.GOV.HK.

## Data Attribution

This extension uses weather observations, forecasts, warning information, and related public weather resources from the Hong Kong Observatory and DATA.GOV.HK.

Relevant source links:

- Hong Kong Observatory Open Data: https://data.weather.gov.hk/
- Hong Kong Observatory: https://www.hko.gov.hk/
- DATA.GOV.HK terms and conditions: https://data.gov.hk/en/terms-and-conditions

The source data and materials remain subject to the applicable terms, notices, copyright, trademark, and attribution requirements of their providers. The extension code is licensed separately under the MIT License.

## Branding

The extension icon and local project artwork are project-specific assets. Do not use the Hong Kong Observatory logo, Government of the Hong Kong Special Administrative Region logos, DATA.GOV.HK marks, or other protected marks in this project unless explicit permission has been obtained and documented.

## Safety

Weather data, warnings, imagery, and notifications may be delayed, unavailable, or incomplete. For official and safety-critical information, always refer to the Hong Kong Observatory and other relevant public authorities.
